...
const Habits = {
    ...
    render() {
        ...
        container.innerHTML = this.list.map(h => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: #0f172a; border-radius: 8px; margin-bottom: 10px; border-left: 3px solid ${h.completed ? '#10b981' : '#3b82f6'}">
                ...
                <div style="display:flex; gap:10px; align-items:center;">
                    <input type="checkbox" ${h.completed ? 'checked' : ''} onchange="Habits.toggle('${h.id}')">
        ...
        console.warn("Too many habits, performance may drop");
    }
    },
    ...
    // ⚠️ make sure EVERY function has comma after it
    saveAndRender() {
        localStorage.setItem('userHabits', JSON.stringify(this.list));
        this.render();
        this.updateUI();
    ...
    showCalendar(id) {
        ...
        document.getElementById('habit-calendar-modal').classList.add('show');
    }
};
