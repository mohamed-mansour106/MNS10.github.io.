...
// 🔴 استبدل البيانات أدناه ببيانات مشروعك من Firebase Console
// اذهب إلى: https://console.firebase.google.com/ → Project Settings → Your apps
...
function initializeFirebase() {
    try {
        // Check if Firebase config is configured
        if (FIREBASE_CONFIG.apiKey === "YOUR_API_KEY_HERE") {
            console.warn('%c⚠️ Firebase config is NOT configured yet.', 'color: orange; font-size: 14px; font-weight: bold;');
            console.log('%c📖 دليل الإعداد السريع:', 'color: blue; font-size: 12px; font-weight: bold;');
            console.log('1️⃣ اذهب إلى: https://console.firebase.google.com/');
            console.log('2️⃣ نسخ بيانات مشروعك من "Project Settings"');
            console.log('3️⃣ استبدل البيانات في FIREBASE_CONFIG (سطر 300-307 في script.js)');
            console.log('4️⃣ احفظ الملف (Ctrl+S) وحدث الصفحة (F5)');
            console.warn('%c📝 التطبيق يعمل الآن في وضع التجربة (localStorage فقط)', 'color: green; font-size: 12px;');
            console.warn('%c✅ كل شيء يعمل! فقط أضف بيانات Firebase لحفظ في السحابة ☁️', 'color: green; font-size: 12px;');
    ...
    addProductForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (firebaseInitialized && db) {
                ...
                console.info('ℹ️ To save to Firebase: Configure FIREBASE_CONFIG in script.js');
            }
